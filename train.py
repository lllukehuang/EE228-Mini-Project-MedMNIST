...
def main(flag, input_root, output_root, end_epoch, download):
    ...
    if flag == 'pathmnist' or flag == 'dermamnist' or flag == 'retinamnist':
    ...
    plt.clf()
    #plt acc
    if not os.path.exists(os.path.join(output_root,flag)):
        os.mkdir(os.path.join(output_root,flag))
    x1 = range(0, epochs)
    y1 = auc_list
    plt.figure()
    plt.plot(x1, y1)
    ...
    plt.xlabel('Epoch')
    plt.xlim(0,epochs)
    plt.savefig(os.path.join(output_root,flag) + '/Acc.jpg')
    # plt.show()

    #plt loss
    x2 = range(0, len(Loss_list))
    y2 = np.array(Loss_list)
    plt.figure()
    plt.plot(x2, y2)
    plt.title('Loss')
    plt.ylabel('Loss')
    plt.xticks([])
    plt.savefig(os.path.join(output_root,flag) + '/Loss.jpg')
    # plt.show()

    #plt loss per epoch
    x3 = range(0, len(Loss_list2))
    y3 = np.array(Loss_list2)
    plt.figure()
    plt.plot(x3, y3)
    plt.title('Loss')
    plt.ylabel('Loss')
    plt.xticks([])
    plt.savefig(os.path.join(output_root, flag) + '/Loss_per_epoch.jpg')
    # plt.show()
...
def test(model, split, data_loader, device, flag, task, output_root=None):
    ...
    with torch.no_grad():
        ...
        res_str = '%s AUC: %.5f ACC: %.5f' % (split, auc, acc)
